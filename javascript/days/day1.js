...
const inputLines = fs.readFileSync("../../input/day1").toString().split("\n");
...
function pt2(inputLine) {
    const matches = Array.from(inputLine.matchAll(/(?=(one|two|three|four|five|six|seven|eight|nine|1|2|3|4|5|6|7|8|9))\w|\d/g))
        ?.map(m => m[1]);
    if (matches.length === 0 ) {
        return 0;
    }
    const first = wordMap[matches[0]] ?? matches[0];
    const last = wordMap[matches[matches.length - 1]] ?? matches[matches.length - 1];
